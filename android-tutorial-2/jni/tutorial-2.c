...
app_function (void *userdata)
{
  ...
  data->pipeline =
          gst_parse_launch
                  //("appsrc name=source ! queue ! udpsink host=192.168.1.23 port=5000", &error);
                  ( "appsrc name=source format=3 ! video/x-h264,width=1920,height=1080,framerate=24/1 ! h264parse ! rtph264pay !  udpsink host=localhost port=5000 sync=false" , &error);
  if (error) {
    gchar *message =
  ...
  g_signal_connect (G_OBJECT (bus), "message",(GCallback) cb_message, data);
...
static void gst_native_receive_video_data(JNIEnv *env, jobject thiz, jbyteArray array) {
  ...
  GstElement *source = gst_bin_get_by_name(GST_BIN(data->pipeline), "source");
...
static JNINativeMethod native_methods[] = {
  ...
  {"receiveVideoData", "([B)V", (void *) gst_native_receive_video_data}
};
...
JNI_OnLoad (JavaVM * vm, void *reserved)
{
  ...
  jclass klass = (*env)->FindClass (env,
      "org/freedesktop/gstreamer/tutorials/tutorial_2/DataFlairService");
